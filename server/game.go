...
func (s *Server) HandleGetGame(w http.ResponseWriter, req *http.Request) {
  ...
  if req.Method != "POST" || req.URL.Path != "/api/game" {
    http.NotFound(w, req)
    return
  }
  ...
  result, err := s.search.GetGames(ids)
  if err != nil {
...
func (s *Server) HandleGameSearch(w http.ResponseWriter, req *http.Request) {
  ...
  if query == "" {
    ...
    result, err = s.search.SearchGame(query)
    if err != nil {
      s.logger.Errorf("failed to execute search query for game using query \"%s\": %s", query, err)
      http.Error(w, "failed to execute query", http.StatusServiceUnavailable)
      return
    }
  }
